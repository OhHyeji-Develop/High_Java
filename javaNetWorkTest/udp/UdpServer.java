...
public class UdpServer {
	...
	/*
	 	UDP 방식 : 비연결 지향, 데이터에 대한 신뢰성이 없다. 데이터가 순서대로 도착한다는 보장이 없다.
	 			  그렇지만 TCP 방식보다 빠르다.
	 			  
	 	- TCP 방식의 경우에는 스트림을 이용해서 송수신을 하고,
	 	  UDP 방식의 경우에는 데이터그램을 이용해서 송수신을 한다.
	...
	public static void main(String[] args) {
		try {
			...
			while(true) {
				...
				// 수신용 패킷객체생성
				// 		=> 데이터가 저장될 byte형 배열, 배열의 길이를 이용하여 DatagramPacket 객체를 생성한다.
...
//				String msg = new String(bMsg, 0, inpacket.getLength(), "utf-8");
				...
				// 송신하기위해 송신용 패킷객체 생성
				// => 전송할 데이터가 저장된 byte배열, 전송할 자료의 길이(=배열의 길이), 상대방의 주소정보, 포트번호 4가지를 지정하여 DatagramPacket객체를 생성
