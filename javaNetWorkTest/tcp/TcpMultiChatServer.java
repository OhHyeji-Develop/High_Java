...
public class TcpMultiChatServer {
	...
	// key값 : 접속한 사람 이름, value값 : 접속한 클라이언트와 연결된 Socket 객체
	...
	class ServerReceiver extends Thread{
		...
		public void run() {
			...
			try {
				while(true) {
					// 클라이언트가 최초로 보낸 데이터는 사용자의 이름인데 이 이름이 중복되는지 여부를 feedback으로 클라이언트에게 보내준다.
					name = dis.readUTF();
					...
					if(clientMap.containsKey(name)){
						...
						// 이름이 중복되지않으면 'OK!'메시지를 보내고 반복문을 탈출한다.
						dos.writeUTF("OK!");
						break;
					}
				} // while문 끝...
				...
				// 대화명(이름)과 클라이언트의 Socket 객체를 Map에 저장
				...
				// 이 finally 영역이 실행된다는 것은 클라이언트가 접속을 종료했을 때 이다.
