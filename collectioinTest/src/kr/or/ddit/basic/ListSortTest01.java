 ...
 *   즉, 외부 정렬기준을 구현할 때 사용하는 인터페이스이다.
 *   
 *  Comparable에서는 compareTo()메서드를 재정의 해야하고,
 *  Comparator에서는 compare()메서드를 재정의 해야한다.
 *  
 *  String 클래스, Wrapper 클래스, Date 클래스, File 클래스 등에는 내부 정렬 기준이 이미 구현되어 있다.
...
public class ListSortTest01 {
	...
	public static void main(String[] args) {
		...
		System.out.println("정렬 전 : " + list);  // [일지매, 홍길동, 성춘향, 변학도, 이순신]
		
		// 정렬은 Collections.sort() 메서드르르 이용하여 정렬한다.
		// Collections.sort() 메서드는 정렬할 데이터의 '내부 정렬기준'을 바탕으로 정렬한다.
		// 보통의 '내부 정렬 기준'은 오름차순으로 정렬되도록 구현되어 있다.
		Collections.sort(list);
		System.out.println("정렬 후 : " + list);  // [변학도, 성춘향, 이순신, 일지매, 홍길동]
		
		Collections.shuffle(list); // 자료 섞기
		System.out.println("자료 섞기 후 : " + list); // 실행할때마다 데이터 순서 바뀜
		
		// 외부 정렬 기준을 지정해서 정렬하기
		Collections.sort(list, new Desc() );
		System.out.println("내림차순 정렬 후 : " + list);  // [홍길동, 일지매, 이순신, 성춘향, 변학도]
	}

}

// 외부 정렬 기준을 정해주는 class작성하기
